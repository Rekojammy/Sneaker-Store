...
fetch("http://localhost:3000/mens-section")
...
fetch("http://localhost:3000/men-cart")
    .then(response => response.json())
    .then(json => {
        json.map(data => {
            // console.log(data)
...
let delCart = {
    apiUrl: "http://localhost:3000/men-cart/{id}",
...
clearCart.addEventListener('click', () => {
    // fetch('http://localhost:3000/men-cart/1,2', {
    //     method: "DELETE",
    //     headers: {
    //         "Content-type": "application/json; charset=UTF-8"
    //     }
...
let store = {
    apiUrl: "http://localhost:3000/mens-section?q={search}",
...
// Search for Category
let catStore = {
    apiUrl: "http://localhost:3000/mens-section?category={category}",
